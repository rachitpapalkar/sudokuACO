        ...
        for row in range(self.grid.grid_size):
            for col in range(self.grid.grid_size):
                ...
                if not sol_cell.failed():
                    self.pher_matrix[row][col][sol_cell.get_val() - 1] = self.pher_matrix[row][col][sol_cell.get_val() - 1] * (1 - self.global_pher_update) + self.global_pher_update * self.best_pher_to_add
